...
class ListIterator {
    ...
    private AddressBook ourBook;

    public ListIterator(AddressBook book){
        ourBook = book;
        reset();
    }

    public void reset(){
        current = ourBook.getFirst();
        previous = null;
    }
    ...
    public void insertFront(Person entry){
        ourBook.setFirst(entry);
    }

    // public Person findLastname(String lname){
    //     this.nextLink();
    //     if(current.returnName().lastName().equals(lname)){
    //         return current;
    //     } else {
    //         return this.findLastname(lname);
    //     }
    // }
